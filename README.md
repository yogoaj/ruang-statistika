...
ruang_statistika/
│
├── app.py                          ← Entry point: routing menu, sidebar, login/personalisasi, CSS global
│
├── modules/
│   ├── __init__.py
│   │
...
│       ├── anova.py                ← ANOVA + Post-hoc (Free: dasar, Pro: Tukey/KW/AI) ★
│       ├── moderasi.py             ← Moderasi/Interaksi + Johnson-Neyman ★
│       ├── logistik.py             ← Regresi Logistik + ROC (Free: dasar, Pro: lengkap) ★
...
│   ├── supabase_auth.py            ← Autentikasi Supabase: Sign In/Up, Google OAuth, Reset Password ← BARU v4.5
...
│
├── .streamlit/
│   └── secrets.toml                ← Kredensial Supabase & app_url (JANGAN diupload ke GitHub!)
│
...
Pengguna Free mendapat **1 laporan per hari**. Kuota dicatat di `.quota_cache.json`. Pengguna Pro tidak terbatas.

### Menambah License Key Baru

Edit `utils/auth.py` → `LICENSE_REGISTRY`:

```python
LICENSE_REGISTRY = {
    "PRO-STAT":       {"expires": None,         "label": "Permanent"},
    "PRO-STAT-2026":  {"expires": "2026-12-31", "label": "Akademisi Pro 2026"},
...
| Kirim ulang konfirmasi | Klik nama user → Resend confirmation |
| User daftar sendiri | Klik tab "Daftar" di modal app |
...
- **Tab Masuk** — email + password, dengan opsi Login via Google
...
2. Tambahkan entry di `app.py` → `MENU_GROUPS` (pilih grup yang sesuai)
...
### v4.5 (Terkini)
- **Auth Supabase:** Login, Registrasi mandiri, Google OAuth, Lupa Password — semua via Supabase Auth
- **`utils/supabase_auth.py`:** Module baru — `supabase_sign_in`, `supabase_sign_up`, `supabase_forgot_password`, `get_supabase_google_oauth_url`, `handle_google_oauth_callback`, `restore_supabase_session`
- **Modal login baru:** 5 tab (Masuk / Daftar / Lupa Password / Kode Pro / Lanjut Gratis)
- **Session restore:** Token Supabase disimpan di `session_state`, di-restore otomatis saat refresh
- **`app.py`:** Inject `handle_google_oauth_callback()` dan `restore_supabase_session()` sebelum render apapun
...
- **Streamlit Secrets support:** `load_users_config()` prioritas baca dari `st.secrets` sebelum file lokal
...
|---|---|---|
| Python | >= 3.10 | Diperlukan untuk `dict \| None` type hint |
| `supabase` | >= 2.0.0 | **WAJIB** untuk sistem auth |
| `kaleido` | >= 0.2.1 | **WAJIB** untuk embed grafik ke Word |
| `bcrypt` | >= 4.0.0 | Untuk fallback auth lokal |
| `pyyaml` | >= 6.0 | Untuk baca users.yaml (fallback) |
...
| GPT-4 (OpenAI) | Berbayar | [platform.openai.com](https://platform.openai.com) |
